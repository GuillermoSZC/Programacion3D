...
Shader::Shader(const std::string& _vertex, const std::string& _fragment)
{
   ...
   normalLoc = glGetAttribLocation(shaderId, "vnormal");
...
void Shader::SetupAttribs() const
{
   if (location != -1)
   {
      ...
      glVertexAttribPointer(location, 3, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<const
   ...
   if (normalLoc != -1)
   {
      ...
      glVertexAttribPointer(normalLoc, 3, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<const
